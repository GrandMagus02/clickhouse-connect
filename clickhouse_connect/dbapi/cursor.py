...
class Cursor:
    ...
    def _try_bulk_insert(self, operation: str, data):
        ...
        if op_columns and {unescape_identifier(x) for x in op_columns} != set(
            col_names
        ):
            return (
                False  # Data sent in doesn't match the columns in the insert statement
            )
    ...
    def executemany(self, operation, parameters):
        ...
        try:
            ...
            raise ProgrammingError(
                f"Invalid parameters {parameters} passed to cursor executemany"
            ) from ex
        self._rowcount = len(self.data)
