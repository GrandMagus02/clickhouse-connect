# clickhouse_connect/integrations.sqla/inspector.py
...
from clickhouse_connect.integrations.sqla.datatypes.base import sqla_type_from_name
from clickhouse_connect.integrations.sqla.ddl.tableengine import build_engine
from clickhouse_connect.integrations.sqla.sql import full_table
from clickhouse_connect.integrations.sqla import dialect_name as dn
...
class ChInspector(Inspector):
    def reflect_table(self, table, include_columns, exclude_columns, *_args, **_kwargs):
        ...
        for col in self.get_columns(table.name, schema):
            ...
            col_args = {
                f"{dn}_{key}" if key in ch_col_args else key: value
...
ChInspector.reflecttable = (
    ChInspector.reflect_table
)  # Hack to provide backward compatibility for SQLAlchemy 1.3
