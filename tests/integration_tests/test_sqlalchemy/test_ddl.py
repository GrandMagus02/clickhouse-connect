"""Tests for SQLAlchemy DDL operations with ClickHouse."""
...
from clickhouse_connect.integrations.sqla.ddl.custom import CreateDatabase, DropDatabase
...
def test_create_database(test_engine: Engine, test_config: TestConfig, test_db: str):
    """Test database creation and dropping.
    ...
    if test_db:
        ...
        if not test_engine.dialect.has_database(conn, create_db):
            if (
                ...
                and conn.connection.connection.client.min_version("20")
            ):
...
class ColorEnum(PyEnum):
    """Enum used for enum column testing."""
...
def test_create_table(test_engine: Engine, test_db: str, test_table_engine: str):
    ...
    # Handle type differences for older ClickHouse versions
    bool_type = Boolean
...
def test_declarative(test_engine: Engine, test_db: str, test_table_engine: str):
    """Test declarative model definition and creation.
